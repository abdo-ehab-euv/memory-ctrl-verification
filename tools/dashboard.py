#!/usr/bin/env python3
"""
tools/dashboard.py — HTML Dashboard Generator
----------------------------------------------
...
# Regex patterns matching scoreboard and coverage output format
...
def analyse(path: str) -> dict:
    """Analyse a single log file and return a results dictionary."""
...
def render_log_card(r: dict) -> str:
    """Render a single log result as an HTML card."""
    ...
    if not r["found"]:
        ...
        <div class="card">
          <h2>{name}</h2>
          <p class="muted">Log file not found at <code>{html.escape(r['path'])}</code>
          — run the corresponding TCL script first.</p>
        </div>"""
    ...
    if r["coverage"]:
        ...
        cov_html = (
            ...
            "<p class='muted'>No COVERAGE_REPORT line in log.</p>"
        )
    ...
    if r["fails"]:
        fails_html = (
            ...
            "<table><tr><th>#</th><th>Test</th><th>Detail</th></tr>"
        )
        for i, f in enumerate(r["fails"], 1):
            fails_html += (
                f"<tr><td>{i}</td>"
                f"<td><code>{html.escape(f['test'])}</code></td>"
